...
## 再開の条件そのものが決まっていない（保留の理由）

- 何が決まれば決まるか: 2026-09-15 にこのアイデアは保留になった（decisions.md）。何が分かれば再開に値するか、オーナーはまだ挙げられていない。候補としては (a) 系統 B の唯一残った作業仮説「当事者としての学習+開発過程の発信」が実際に機能しそうだと分かる、(b) 体験の像が別の形で結ぶ、(c) english-vocab-app の実装を通じて「手を動かして分かること」が増える、の 3 つがあるが、どれも決定ではない。
- 材料: decisions.md 2026-09-15「保留」。系統 A / B / C の整理（上記）。同日に却下した english-opinion-trainer の失敗（体験としてのまとまりを別に設計しないと出てこない）。
- 状態: 保留（再開条件が未決であること自体を、この項目で明示している）
