...
| `english-saas-venture` | 検討中 | 個人事業として成立する語学学習アプリを探す検討。痛みは日常会話で言いたいことを切り下げてしまうこと。自分の誤りを起点に「同じ型の別問題」を出し、転移＝前に詰まった課題を別の問題でクリアできたことを達成として返す（デスクトップのウェブ、3〜5 分、テキスト産出、最初は英語のみ・仕組みは言語不問）。`overview.md` / `experience.md` あり。**未決は 2 系統に分割済み（open-questions.md）。系統 A＝体験・課題の再検討、系統 B＝ユーザー獲得。別セッションで再開する。** 獲得経路は調査 13 本を経ても機能する経路が未発見 |
